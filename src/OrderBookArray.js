...
class OrderBook extends Emitter {
  ...
  _dedupArray (obSideOld, obSideData, lastUpdateId) {
    const obSideNew = obSideData.map(x => new OrderBookRecord(x[0], x[1], lastUpdateId))
    return [...obSideNew, ...obSideOld].filter(excludeDupes)
  }
  ...
  _getUpdatedOBSide (obSideOld, obSideData, lastUpdateId, sortFn) {
    ...
    return this._dedupArray(obSideOld, obSideData, lastUpdateId).filter(excludeNoVolume).sort(sortFn)
  }
  ...
  getTopNAsStr (side, n) {
    ...
    if (side === Side.BID) {
      return getOBTopNAsStr(this.bids, n)
    }
    if (side === Side.ASK) {
      return getOBTopNAsStr(this.asks, n)
    }
  }
}
